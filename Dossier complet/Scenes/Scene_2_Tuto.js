class Scene66 extends Phaser.Scene {
  constructor() {
    super("Tuto");
  }
    ...
    this.load.image('level2','assets/monde/level2.png');
    this.load.image('arriere','assets/monde/champ.png');
  ...
  create(){
    ...
    this.physics.world.setBounds(0, 0, 1600, 600);
  	this.add.image(0,0,'arriere').setOrigin(0,0);
    this.add.image(1300,20,'info');
    this.add.image(1120,20,'info');
    this.add.image(610,26,'info');
    this.add.image(300,26,'info');
    this.add.image(1000,300,'info');

  	this.barrières = this.physics.add.staticGroup();
    this.barrières.create(48,564, 'platform').setScale(1,15).refreshBody();
    this.barrières.create(1548,564, 'platform').setScale(1,15).refreshBody();
    this.barrières.create(496,400, 'platform').setScale(20,4.8).refreshBody();
		this.barrières.create(1110,600, 'platform').setScale(20,4.8).refreshBody();
    ...
    this.door.create(130,600,'chateau');
    ...
    this.cube = this.physics.add.sprite(950,230,'cube');
		this.cube.setCollideWorldBounds(true);
    this.cube.pointVie=1;


    this.bonhomme = this.physics.add.sprite(350,300,'bonhomme');
		this.bonhomme.setCollideWorldBounds(true);
    this.bonhomme.pointDeVie=1;
    ...
    this.cibles.children.iterate(function (cible) {
      cible.pointsVie=1;
      cible.y = (Phaser.Math.Between(20,260));
    });
    ...
    this.inventText = this.add.text(500, 2, "Change d'objet \navec C ou V", {'font': '14px', fill: '#000'});
    this.inventText.visible = false;

    this.useText = this.add.text(200, 2, "Utilise tes objets \navec S", {'font': '14px', fill: '#000'});
    this.useText.visible = false;

    this.pushText = this.add.text(1200, 2, "Maintient O pour intéragir\n(Passer des portes/Lire/Parler)", {'font': '14px', fill: '#000'});
    ...
    this.bloqueText = this.add.text(800, 250, "Si jamais tu as fait le mauvais choix\nOu que tu as une mauvaise strat\nUtilise la touche ENTREE et tu reprendras le niveau", {'font': '14px', fill: '#000'});
    this.bloqueText.visible = false;

    this.porteText = this.add.text(0, 450, "A l'avenir,\nfait comme prévu \net cliques sur O", {'font': '14px', fill: '#000'});
    ...
    this.physics.add.overlap(this.player,this.cube, hitcube, null, this);
    this.physics.add.overlap(this.player,this.bonhomme, hitbonhomme, null, this);
    ...
    if(Phaser.Input.Keyboard.JustDown(this.s)){
      ...
      if (Phaser.Input.Keyboard.JustDown(this.v) && this.choix < 4){
        ...
        this.choixText.setText('Choix: '+ (this.choix));
          ...
          //Textes
      if(this.player.x > 1000 && this.player.x < 1200 && this.keys.O.isDown){
      ...
      if(this.player.x > 1400 && this.player.x < 1500 && this.keys.O.isDown){
        if(this.player.y > 200 && this.score < 30){
      ...
      if(this.player.x > 200 && this.player.x < 400 && this.keys.O.isDown){
      ...
      if(this.player.x > 500 && this.player.x < 700 && this.keys.O.isDown){
        this.inventText.visible = true;
      }
      ...
      if(this.player.x > 1200 && this.player.x < 1400 && this.keys.O.isDown){
        if(this.player.y<200){
          this.fouilleText.visible = true;
        }
        this.pushText.visible = false;
  		}
      ...
      if(this.player.x > 900 && this.player.x < 1100 && this.keys.O.isDown){
        if(this.player.y > 200){
          this.bloqueText.visible = true;
        }
  		}
